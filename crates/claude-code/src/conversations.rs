...
mod tests {
    ...
    fn list_conversations_handles_malformed_first_line() {
        ...
        write_jsonl(dir.path(), "malformed.jsonl", &["not json at all", &good]);
    ...
    fn list_conversations_skips_memory_subdir() {
        ...
        fs::write(dir.path().join("memory").join("summary.jsonl"), "ignored\n").unwrap();
    ...
    fn list_conversations_falls_back_to_mtime_when_timestamp_missing() {
        ...
        let raw =
            r#"{"parentUuid":null,"type":"user","message":{"role":"user","content":"no ts"}}"#;
