...
"#,
    ...
    ///
    /// Mirrors the helper in Claude's `conversations.rs` tests: uses
    /// `File::set_modified` (stable since Rust 1.75) so the tests behave
    /// identically on every CI platform — the previous unix-only
    /// `utimes(2)` version silently no-opped on Windows and let stale
    /// fixtures look fresh.
