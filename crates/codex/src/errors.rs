//! Stderr-pattern classification for Codex CLI invocations.
//!
//! Codex returns unstructured stderr on failure. We scan for a small set
//! of known substrings to distinguish auth / rate-limit failures from
//! everything else, so the UI can surface actionable guidance. Unknown
//! errors fall through to [`AiError::Other`] with the raw stderr attached.
//!
//! Patterns are case-insensitive and conservative — if Codex changes its
//! wording, the classifier quietly reports `Other(...)` instead of
//! misclassifying.
...
pub fn classify_stderr(stderr: &str) -> Option<AiError> {
    ...
    if lower.contains("command not found") || lower.contains("no such file or directory") {
        return Some(AiError::BinaryNotFound("codex".into()));
    }
...
mod tests {
    ...
    fn binary_not_found_pattern() {
        let err = classify_stderr("codex: command not found").unwrap();
