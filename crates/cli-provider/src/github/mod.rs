...
//! [`releases`]). The submodules define `impl GitHubCli { pub(super) fn
...
use forge_provider::{
    ...
    IssueDetail, IssueFilter, Label, MergeStrategy, Milestone, MrPr, MrPrDetail, MrPrDiffFile,
    MrPrFilter, Release, ReleaseAsset, ReleaseDetail,
};
...
///
/// Kept at module root because it's shared between `list_mr_prs_impl` and any
/// other MR/PR-state-sensitive helpers that might be added later.
pub(super) fn state_to_gh_str(s: forge_provider::MrPrState) -> &'static str {
    match s {
